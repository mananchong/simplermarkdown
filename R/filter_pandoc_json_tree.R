...
filter_pandoc_json_tree <- function(con) {
  ...
  # evaluate the code
  dta$blocks <- parse_blocks(dta$blocks,
...
parse_blocks <- function(blocks, verbosity = 1, default_fun = "output_eval",
  ...
  types_to_ignore <- c(
    "Str", "Space", "Str", "Strong", "Link", "Image",
    "Table", "SoftBreak"
  )
  ...
  for (i in seq_along(blocks)) {
    ...
    if (block$t == "Code") {
      ...
      block$c[[3]] <- parse_blocks(
        block$c[[3]], verbosity, default_fun,
        eval_block, eval_inline
      )
    } else if (block$t == "BulletList") {
      for (j in seq_along(block$c)) {
        block$c[[j]] <- parse_blocks(
          block$c[[j]], verbosity, default_fun,
          eval_block, eval_inline
        )
      }
    } else if (block$t == "OrderedList") {
      for (j in seq_along(block$c[[2]])) {
        block$c[[2]][[j]] <- parse_blocks(
          block$c[[2]][[j]], verbosity, default_fun,
          eval_block, eval_inline
        )
      }
    } else if (block$t == "Div") {
      block$c[[2]] <- parse_blocks(
        block$c[[2]], verbosity, default_fun,
        eval_block, eval_inline
      )
    } else if (block$t %in% basic_types) {
      block$c <- parse_blocks(
        block$c, verbosity, default_fun,
        eval_block, eval_inline
      )
